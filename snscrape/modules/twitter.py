...
class Tweet(snscrape.base.Item):
	...
	outlinksss = snscrape.base._DeprecatedProperty('outlinksss', lambda self: ' '.join(x.url if x.url else '' for x in self.links) if self.links else '', 'links (url attribute)')
	...
	tcooutlinksss = snscrape.base._DeprecatedProperty('tcooutlinksss', lambda self: ' '.join(x.tcourl if x.tcourl else '' for x in self.links) if self.links else '', 'links (tcourl attribute)')
				...
				if entry['entryId'].startswith('tweet-'):
					...
					for item in entry['content']['items']:
						if item['entryId'].startswith(f'{entry["entryId"]}-tweet-'):
							...
							yield self._graphql_timeline_tweet_item_result_to_tweet(item['item']['itemContent']['tweet_results']['result'], tweetId = tweetId, **kwargs)
				elif not entry['entryId'].startswith(('cursor-', 'toptabsrpusermodule-', 'tweetdetailrelatedtweets-', 'label-', 'promoted-tweet-', 'who-to-follow-')):
